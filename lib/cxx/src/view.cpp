...
EMSCRIPTEN_BINDING_BEGIN(AView) {
  emscripten::class_<AView, emscripten::base<TreeNodeElement>>("AView")
      .constructor<core::NodePtr_t, core::NodeIndex, std::string, TreeNodeElementDescriptor>()
...
AView::AView(core::NodePtr_t parent, const core::NodeIndex &nodeIndex, const std::string &nodeId,
    const TreeNodeElementDescriptor &createInfo)
    : TreeNodeElement(
          // parent, nodeIndex, nodeId,
          createInfo)
    , _styleSheet(createInfo.stylesheet) {
...
auto AView::traverse(core::typedefs::TraverserSharedPtr_t traverser) -> u32_t {
...
StyleSheet AView::getStyleSheet() const { return _styleSheet; }

void AView::setStyleSheet(const emscripten::val &mapper) { _styleSheet = StyleSheet(mapper); }
  ...
  for (std::shared_ptr<Selector> selector : getSelectors()) {
    if (selector->getType() == SelectorType::CN) {
      ...
      for (std::string mod : cnSelector->getMods())
        classnameList.push_back(
            _styleSheet.getClassName(core::format("%s--%s", cnSelector->getName().c_str(), mod.c_str())));
    }
  }
...
void AView::addSelector(std::shared_ptr<Selector> selector) { _selectors.push_back(selector); }

auto AView::getSelectors() -> std::vector<std::shared_ptr<Selector>> { return _selectors; }

core::Observable *AView::getModel() { return _model; }
